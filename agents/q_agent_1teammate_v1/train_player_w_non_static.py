...
def export_metrics(trained_eps: list, rewards: list, epsilons: list,
                   q_variation: list, lrs: list, save_dir: str):
...
if __name__ == '__main__':
    ...
    agent = QLearningAgent(num_states=features_manager.get_num_states(),
                           ...
                           epsilon=0.6, final_epsilon=0.1)
    agent.load_q_table(model_file)
    # save original q_table
    save_model(q_table=agent.q_table, file_name="original_model",
               directory=save_dir)
    ...
    learning_rates = [agent.learning_rate]
    ...
    export_metrics(trained_eps=trained_eps_list, rewards=avr_rewards_list,
                   epsilons=avr_epsilons_list, lrs=learning_rates,
