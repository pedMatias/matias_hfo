...
from agents.offline_plastic_v1.deep_agent import DQNAgent
...
class Player:
    ...
    def test(self, num_episodes: int, start_with_ball: bool = True,
             starts_fixed_position: bool = True) -> (float, float, float):
        """
        ...
        for ep in range(num_episodes):
            ...
            self.set_starting_game_conditions(
                ...
                starts_fixed_position=starts_fixed_position, verbose=False)
...
if __name__ == '__main__':
    ...
    if choose_best_sub_model:
        ...
        avr_passed_ball_rate = player.test(
            num_episodes,
        ...
        if not os.path.isfile(stage_model_file) or prev_metrics is None:
            ...
            os.remove(model_file)
                ...
                and correct_actions_rate >= 0.6 \
                and avr_touched_ball >= min(0.3, prev_metrics["touched_ball_rate"]):
            test_metrics[f"stage_{stage}"] = {
                "win_rate": avr_win_rate,
                "best_model": test_iter,
                "correct_actions_avr": correct_actions_rate,
            ...
            os.remove(model_file)
                ...
                and avr_win_rate >= (prev_metrics["win_rate"] - 0.2) \
                and avr_win_rate >= 0.2 \
                and correct_actions_rate >= 0.6 \
                and avr_touched_ball >= min(0.3, prev_metrics["touched_ball_rate"]):
            test_metrics[f"stage_{stage}"] = {
                "win_rate": avr_win_rate,
        ...
        # More correct actions selected:
                ...
                and avr_win_rate >= max((prev_metrics["win_rate"] - 0.1), 0.2)\
                and avr_passed_ball_rate >= min(prev_metrics["avr_passed_ball_rate"], 0.2) \
                and avr_touched_ball >= min(0.3, prev_metrics["touched_ball_rate"]):
            test_metrics[f"stage_{stage}"] = {
                "win_rate": avr_win_rate,
                "best_model": test_iter,
