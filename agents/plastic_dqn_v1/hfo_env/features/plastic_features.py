...
class PlasticFeatures(BaseHighLevelState):
    ...
    # (goal open, distance to op, pass angle, has ball)
    num_teammate_features = 4
    ...
    def __init__(self, num_team: int = None, num_op: int = None):
        ...
        # Coordinates:
        self.a_coord: np.ndarray = np.array([0, 0])
        self.t_coord: np.ndarray = np.array([0, 0])
        self.ball_coord: np.ndarray = np.array([0, 0])
    ...
    def update_features(self, observation: list):
        ...
        # Coordinates:
        self.a_coord: np.ndarray = self.get_agent_coord()
        self.t_coord: np.ndarray = self.get_teammate_coord()
        self.near_op_coord = self.get_nearest_opponent_coord()
        self.ball_coord: np.ndarray = self.get_ball_coord()
        ...
        for teammate in self.teammates:
            ...
            idx += 1
            self.features_vect[idx] = 1 if self.teammate_has_ball() else -1
            idx += 1
    ...
    def teammate_has_ball(self) -> bool:
        teammate_dist_to_ball = np.linalg.norm(self.t_coord - self.ball_coord)
        teammate_near_ball = True if teammate_dist_to_ball <= 0.1 else False
        return True if teammate_near_ball and not self.has_ball() else False
