...
def has_better_pos(dist_to_op, goal_angle, pass_angle, curr_goal_angle):
    ...
    if curr_goal_angle > goal_angle:
        # print(f"Not Better Pos -> curr_goal_angle={curr_goal_angle} > "
        #      f"goal_angle={goal_angle};")
        return False
    if dist_to_op < params['DRIB_DST']:
        # print(f"Not Better Pos -> dist_to_op={dist_to_op};")
        return False
    if pass_angle < params['PASS_ANG']:
        # print(f"Not Better Pos -> pass_angle={pass_angle};")
        return False
    return True
...
def main():
    ...
    parser.add_argument('--seed', type=int, default=None,
                        help="Python randomization seed; uses python default "
                             "if 0 or not given")
    parser.add_argument('--rand-pass', action="store_true",
                        help="Randomize order of checking teammates for a "
                             "possible pass")
    parser.add_argument('--epsilon', type=float, default=0,
                        help="Probability of a random action if has the ball, "
    ...
    hfo_env.connectToServer(hfo.HIGH_LEVEL_FEATURE_SET,
                            '/home/matias/Desktop/HFO/bin/teams/base/'
    ...
    if args.seed:
        if (args.rand_pass and (num_teammates > 1)) or (args.epsilon > 0):
            ...
            print(
                "Python randomization seed useless without --rand-pass w/2+ "
    ...
    for episode in itertools.count():
        ...
        # if args.epsilon > 0:
        #     print(
        #         "\tNum move: {0:d}; Random action: {1:d}; Nonrandom: "
        #         "{2:d}".format(num_move, num_eps, (num_had_ball - num_eps)))
