...
if __name__ == '__main__':
    ...
    lr = 0.0005
    n_games = 5
    agent = DeepQAgent(gamma=0.99, epsilon=0, alpha=lr, input_dims=4,
    ...
    for i in range(n_games):
        ...
        while status == IN_GAME:
            action_idx = agent.select_action(env.get_representation())
            ...
            if status == SERVER_DOWN:
                hfo.act(QUIT)
                break
            score += get_reward(status, env.agent.can_kick, action_id)
            new_observation = hfo.getState()
            env = EasyState(new_observation, num_team=num_teammates,
                                num_op=num_opponents)
        scores.append(score)
