    ...
    def export(self, verbose: bool = True):
        ...
        for ep_guesses in self.guessed_teams:
            total_correct_perct += ep_guesses.count(self.correct_team) / \
                                   len(ep_guesses)
            if ep_guesses.count(self.correct_team) > 0:
                avg_time_to_detect += ep_guesses.index(self.correct_team) / \
                                      len(ep_guesses)
        total_correct_perct = total_correct_perct / len(self.guessed_teams)
        avg_time_to_detect = avg_time_to_detect / len(self.guessed_teams)
        ...
        if verbose:
            print(f"[Game Metrics: Summary] WIN rate = {win_rate}; "
                  ...
                  f"Average time to detect team = {avg_time_to_detect};")
        return win_rate, correct_actions_rate, \
               avr_touched_ball_rate, avr_passed_ball_rate, \
               total_correct_perct, avg_time_to_detect
    
    def export_to_dict(self, verbose: bool = True):
        win_rt, corr_act_rt, avr_ball_rt, avr_pass_rt, corr_guess, avr_ttd = \
