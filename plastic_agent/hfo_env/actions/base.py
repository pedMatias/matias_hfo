...
import numpy as np
from hfo import MOVE_TO, DRIBBLE_TO, KICK_TO, NOOP, GO_TO_BALL, INTERCEPT
...
from plastic_agent.utils import get_angle
...
class BaseActions(ABC):
    ...
    def move_to_ball(self, num_rep: int = 1):
        ...
        while self.game_interface.in_game() and attempts < num_rep:
            dist_to_ball = abs(np.linalg.norm(self.features.a_coord -
                                              self.features.t_coord))
            if (self.features.teammate_has_ball() and dist_to_ball <= 0.3) \
                    or self.features.has_ball():
                return self.move_to_goal(num_rep=1)
    ...
    def move_to_goal(self, num_rep: int = 1):
        ...
        while self.game_interface.in_game() and attempts < num_rep:
            ...
            if abs(np.linalg.norm(self.features.a_coord - goal_coord)) <= 0.15:
                break
